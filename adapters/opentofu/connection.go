...
type opentofuConnection struct {
	cred credentials.Credential
}

func (c *opentofuConnection) Execute(ctx context.Context, op provider.Operation) (provider.Result, error) {
	switch op.Action {
	...
	default:
		return provider.Result{}, &provider.PlatformError{
			...
			Message:   fmt.Sprintf("OpenTofu provider does not support action: %s", op.Action),
		}
	}
}

func (c *opentofuConnection) Close() error {
	return nil
}

func (c *opentofuConnection) checkBinary(ctx context.Context) error {
	cmd := exec.CommandContext(ctx, "tofu", "--version")
	return cmd.Run()
}

func (c *opentofuConnection) runCommand(ctx context.Context, op provider.Operation, args ...string) (provider.Result, error) {
	...
	if c.cred != nil {
		...
		if err != nil {
			return provider.Result{}, &provider.PlatformError{
				...
				Message:   "Failed to apply credentials for OpenTofu execution",
				UnderlyingCause: err,
			}
		}
